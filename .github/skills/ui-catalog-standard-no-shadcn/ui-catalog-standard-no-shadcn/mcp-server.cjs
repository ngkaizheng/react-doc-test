...
try {
    ...
    console.error(
        "[MCP] Loaded tool:",
        specTool.name || "get_component_spec"
    );

} catch (e) {

    console.error(
        "[MCP] Failed loading get-spec.cjs:",
        e
    );
}
            ...
            tools: [
                {
                    ...
                    description:
                        specTool?.description ||
                        "获取指定共享UI组件规范",

                    inputSchema:
                        specTool?.inputSchema ||
                        {
                            type: "object",
...
server.setRequestHandler(
    ...
    async (request) => {
        ...
        try {
            ...
            if (
                !specTool ||
                typeof specTool.handler !== "function"
            ) {

                throw new Error(
                    "get-spec.cjs missing handler export"
                );
            }
                ...
                content: [
                    {
                        type: "text",
                        text:
                            `Error executing spec: ${err.message}`
                    }
                ],
